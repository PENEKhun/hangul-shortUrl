...
public class ShortController {
  ...
  public ResponseEntity makeURL(HttpServletRequest request) {
    ...
    try {
      ...
      if (originUrl.isEmpty()) {
        throw new CustomException(ErrorCode.NO_INPUT);
      }
  ...
  public ResponseEntity<String> hangulToUrl(@PathVariable String hangul) {
    return redirectToOriginalUrl(RequestInformation.parse(), hangul);
  }

  private ResponseEntity<String> redirectToOriginalUrl(RequestInformation request, String hangul) {
    String originUrl = shortService.restoreToOriginalUrl(request, hangul);
